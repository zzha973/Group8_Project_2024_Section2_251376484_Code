...
def train_and_evaluate(model, train_loader, val_loader, optimizer, scheduler, device, epochs):
    ...
    for epoch in range(epochs):
        ...
        if val_accuracy > best_accuracy:
            ...
            torch.save(model.state_dict(), 'roberta-large.pt')
            print("Model saved!")
            print("\n")
        else:
...
def main():
    ...
    model = RobertaForSequenceClassification.from_pretrained('roberta-large', num_labels=4).to(device)
    ...
    plt.title('roberta-large_accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')

    plt.grid(True)
    plt.show()
    plt.savefig('roberta-large_accuracy.png', bbox_inches='tight')
