...
def train_and_evaluate(model, train_loader, val_loader, optimizer, scheduler, device, epochs):
    ...
    for epoch in range(epochs):
        ...
        if val_accuracy > best_accuracy:
            ...
            torch.save(model.state_dict(), 'deberta-large.pt')
            print("Model saved!")
            print("\n")
        else:
...
def main():
    ...
    model = DebertaForSequenceClassification.from_pretrained('microsoft/deberta-large', num_labels = 4).to(device)
    ...
    train_loader = DataLoader(tokenized_datasets['train'], batch_size=64, shuffle=True, num_workers=4)
    val_loader = DataLoader(tokenized_datasets['test'], batch_size=64,num_workers=4)
    ...
    plt.title('deberta-large_accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')

    plt.grid(True)
    plt.show()
    plt.savefig('deberta-large_accuracy.png', bbox_inches='tight')
