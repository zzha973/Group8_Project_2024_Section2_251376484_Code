...
def train_and_evaluate(model, train_loader, val_loader, optimizer, scheduler, device, epochs):
    ...
    for epoch in range(epochs):
        ...
        if val_accuracy > best_accuracy:
            ...
            torch.save(model.state_dict(), 'electra-large.pt')
            print("Model saved!")
            print("\n")
        else:
...
def main():
    ...
    model = ElectraForSequenceClassification.from_pretrained('google/electra-large-discriminator', num_labels=4).to(device)
    ...
    plt.title('electra-large_accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    ...
    plt.grid(True)
    plt.show()
    plt.savefig('electra-large_accuracy.png', bbox_inches='tight')
